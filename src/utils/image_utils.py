...
def save_depth(images, fname, text_list=[None], merge=1, col=8, scale=False):
    """
    :param it:
    :param images: Tensor of (N, C, H, W)
    :param text_list: str * N
    ...
    nonzeros = images[images>0]
    if(len(nonzeros)>0):
        min_val, max_val = nonzeros.min(), nonzeros.max()
    else:
        min_val, max_val = 0, 0
    ...
    for i, img in enumerate(images):
        ...
        if len(nonzeros) > 0:
            nonzeros = 1-(nonzeros - min_val) / (max_val - min_val)
            nonzeros = np.clip(255 * nonzeros, 0, 255).astype(np.uint8)
            # img[img>0] = nonzeros
            nonzeros = cv2.applyColorMap(nonzeros, cv2.COLORMAP_JET)
            if nonzeros is not None:
                blank_image[img>0] = nonzeros.reshape(-1, 3)
        blank_image = put_multi_line(blank_image, text_list[i])
        image_list.append(blank_image)
    merge_image = np.array(image_list).astype(np.float32)
    merge_image = merge_image.transpose([0, 3, 1, 2]) # [N, C, H, W]
    
    merge_image = vutils.make_grid(torch.from_numpy(merge_image), nrow=col)  # (C, H, W)
    merge_image = merge_image.numpy().transpose([1, 2, 0]).astype(np.uint8)
